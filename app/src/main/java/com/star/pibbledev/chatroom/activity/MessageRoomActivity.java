...
import com.star.pibbledev.chatroom.fragment.MessageRoomFragment;
...
public class MessageRoomActivity extends BaseActivity implements View.OnClickListener, RequestListener, MessageRoomListAdaptar.MessageRoomListListener {
    ...
    boolean isAddedScrollListener = false;

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.fragment_message_room);
    ...
    private void getDigitalgoodsRooms(int page) {
        ...
        if (Constants.isLifeToken(this)) {
            ...
            ServerRequest.getSharedServerRequest().getDigitalgoodsRooms(this, this, token, String.valueOf(page), "5");
    ...
    private void getPersonalRooms(int page) {
        ...
        if (Constants.isLifeToken(this)) {
            ...
            ServerRequest.getSharedServerRequest().getPersonalRooms(this, this, token, String.valueOf(page), "5");
    ...
    private void leaveRoom(int roomid) {
        ...
        if (Constants.isLifeToken(this)) {
            ...
            String token = Utility.getReadPref(this).getStringValue(Constants.AUTH_ACCESS_TOKEN);
            ServerRequest.getSharedServerRequest().deleteChatRoom(this, this, token, roomid, roomModels.get(mCurrentPage).isMute);
        ...
        if (groupAry != null && groupAry.length() > 0) {
            for (int i = 0; i < groupAry.length(); i++) {
                ...
                try {
                    ...
                    roomModel.isSeller = groupObj.optInt("owner") == Utility.getReadPref(this).getIntValue(Constants.ID);
...
//            name = roomModels.get(position).lastName + " " + roomModels.get(position).firstName;
        ...
        intent.putExtra(Constants.TARGET, MessageRoomFragment.TAG);
    ...
    public void onClickGroupInfo(int position) {
        ...
        @SuppressLint("DefaultLocale") String detail = String.format("%s %d %s, %s %.2f %s", getString(R.string.price), roomModels.get(position).goodsPrice, getString(R.string.pib), getString(R.string.reward) ,roomModels.get(position).goodsPrice * 0.15, getString(R.string.pib));
